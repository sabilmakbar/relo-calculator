...
Because deployed free hosts get 503'd, cross-country comparisons in production are served from [`../app/data/numbeo_cache.json`](../app/data/numbeo_cache.json) — city-pair results pre-scraped **locally from a clean residential IP** by [`../scripts/build_cache.py`](../scripts/build_cache.py) and committed to the repo (so they survive the container's ephemeral filesystem and every spin-down). `get_percentage_diff` checks the cache first (direction-sensitive, case-normalised key) and only falls back to a live scrape for an un-cached pair — which then degrades to a "temporarily unavailable" notice if the host is blocked. To extend coverage, edit the `CITIES` list in the script, re-run it, and commit the updated JSON. COL data moves slowly, so a snapshot stays valid for months.
