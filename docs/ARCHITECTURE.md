...
Because deployed free hosts get 503'd, cross-country comparisons in production are served from [`../app/data/numbeo_cache.json`](../app/data/numbeo_cache.json) — pre-built **locally from a clean residential IP** by [`../scripts/build_cache.py`](../scripts/build_cache.py) and committed to the repo (so it survives the container's ephemeral filesystem and every spin-down). `get_percentage_diff` checks the cache first (direction-sensitive, case-normalised key) and only falls back to a live scrape for an un-cached pair — which then degrades to a "temporarily unavailable" notice if the host is blocked.

**How it's built — the index trick.** Numbeo's pairwise percentages are *reciprocal and transitive* (verified), i.e. they all derive from a single cost index per city. So rather than scrape N×(N−1) pairs (8,000+ for the whole dropdown — infeasible under the ~50-request rate limit), the builder scrapes each capital **once** against a fixed reference city (Singapore), stores its index in [`../app/data/numbeo_index.json`](../app/data/numbeo_index.json), and *computes* every ordered pair: `valuePct(A→B) = (index_B / index_A − 1) × 100`. That's ~91 requests for full coverage instead of thousands. The rate limiter still caps each run at ~50 cities, so the builder is **resumable** — it skips capitals already in the index; re-run it (spaced out, as limits reset) until the index is complete, then commit both JSON files. COL data moves slowly, so a snapshot stays valid for months.
